...
function Tutoring() {
  ...
  }, [search, selectedFormat, minRate, maxRate]);


  const summaryMinRate = Math.min(minRate, maxRate);
  const summaryMaxRate = Math.max(minRate, maxRate);
    ...
    <main className="tutoring-page">
      ...
      <section className="tutoring-filters" aria-label="Tutoring filters">
        ...
        <p className="rate-summary">
          Showing tutors between <strong>${summaryMinRate}</strong> and
          <strong> ${summaryMaxRate}</strong> / hour.
        </p>
      </section>
