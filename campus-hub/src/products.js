import bookImg from "./assets/book.jpg";
import book2Img from "./assets/book2.jpg";
import organicChemImg from "./assets/organic-chem.svg";
import graphingCalculatorImg from "./assets/graphing-calculator.svg";

const placeholder = (id, w = 400, h = 300) =>
  `https://images.unsplash.com/photo-${id}?w=${w}&h=${h}&fit=crop`;

export const products = [
  {
    id: 1,
    productName: "Thomas Calculus 11th Edition",
    author: "Sarah Williams",
    price: 55,
    description: "Brand new. No writings.",
    category: "Books",
    productImage: bookImg,
  },
  {
    id: 2,
    productName: "Probability & Random Processes",
    author: "James Hugh",
    price: 49,
    description: "Fairly used, good condition.",
    category: "Books",
    productImage: book2Img,
  },
  {
    id: 3,
    productName: "Vintage Hoodie",
    author: "Alex Brown",
    price: 30,
    description: "Comfortable and warm.",
    category: "Clothing",
    productImage: placeholder("1556821840-0aaf85586236"),
  },
  {
    id: 4,
    productName: "Desk Lamp",
    author: "Sarah Williams",
    price: 25,
    description: "Perfect for study desks.",
    category: "Room Decor",
    productImage: placeholder("1524484485835-2a3e2c8e8c3a"),
  },
  {
    id: 5,
    productName: "Organic Chemistry 2nd Ed",
    author: "John Doe",
    price: 42,
    description: "Sample description of the item's current state.",
    category: "Books",
    productImage: organicChemImg,
    discountPercent: 50,
  },
  {
    id: 6,
    productName: "Graphing Calculator",
    author: "John Doe",
    price: 35,
    description: "Sample description of the item's current state.",
    category: "Books",
    productImage: graphingCalculatorImg,
    discountPercent: 56,
  },
  {
    id: 7,
    productName: "Linear Algebra Done Right",
    author: "Mia Chen",
    price: 38,
    description: "Lightly used with clean pages.",
    category: "Books",
    productImage: placeholder("1512820790803-83ca734da794"),
  },
  {
    id: 8,
    productName: "Campus Crewneck Sweatshirt",
    author: "Noah Patel",
    price: 28,
    description: "Soft fleece interior, worn twice.",
    category: "Clothing",
    productImage: placeholder("1521572163474-6864f9cf17ab"),
  },
  {
    id: 9,
    productName: "Denim Jacket",
    author: "Olivia Reed",
    price: 40,
    description: "Classic fit in excellent condition.",
    category: "Clothing",
    productImage: placeholder("1521223890158-f9f7c3d5d504"),
  },
  {
    id: 10,
    productName: "Monstera Plant in Ceramic Pot",
    author: "Ethan Brooks",
    price: 22,
    description: "Healthy indoor plant, easy to maintain.",
    category: "Room Decor",
    productImage: placeholder("1463320726281-696a485928c7"),
  },
  {
    id: 11,
    productName: "Framed City Skyline Print",
    author: "Ava Martinez",
    price: 18,
    description: "Modern black frame, ready to hang.",
    category: "Room Decor",
    productImage: placeholder("1500530855697-b586d89ba3ee"),
  },
  {
    id: 12,
    productName: "Bedside Storage Caddy",
    author: "Liam Carter",
    price: 14,
    description: "Fits dorm bed rails and holds essentials.",
    category: "Room Decor",
    productImage: placeholder("1484101403633-562f891dc89a"),
  },
];
